...
import { Calendar, User, CheckCircle2, Circle, ChevronDown, ChevronUp } from 'lucide-react';
...
export default function TaskCard({ task }: Props) {
  ...
  return (
    <div className={`rounded-lg border transition-all duration-200 hover:shadow-md ${task.completed
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <h3 className={`text-sm font-semibold ${task.completed ? 'text-gray-500 line-through' : 'text-gray-900'
            }`}>
            {task.title}
          </h3>
          ...
          <div className="flex items-center gap-3 mt-2 text-xs text-gray-500">
            ...
            <div className="flex items-center gap-1">
              ...
              <span className="truncate max-w-[100px]">{task.owner.name}</span>
            </div>
          </div>
        </div>
